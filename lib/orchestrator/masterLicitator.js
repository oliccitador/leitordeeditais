...
// ✅ SANDBOX: Usando V2 isolado para correção de bug sem afetar legado
import ItemClassifier from '../agents/03-items-v2.js';
...
class MasterLicitator {
    constructor() {
        ...
        // Inicializa Agentes (instância única por batch) - Importante para reutilização se necessário
    ...
    async process(CORPO_INTEGRADO, userContext = {}) {
    ...
    async execute(files, userQuestions = [], userContext = {}, cnpj = null) {
        try {
            try { this.status = 'running'; } catch (e) { }
            ...
            const complianceResult = await this.executeAgent(
                ...
                () => this.runComplianceChecker(this.safeClone(CORPO_INTEGRADO))
            );
            ...
            const technicalResult = await this.executeAgent(
                ...
                () => this.runTechnicalValidator(this.safeClone(CORPO_INTEGRADO))
            );
            ...
            const divergenceResult = await this.executeAgentSafe(
                ...
                () => this.runDivergenceScanner(this.safeClone(CORPO_INTEGRADO))
            );
            ...
            const legalResult = await this.executeAgentSafe(
                ...
                () => this.runLegalMindEngine(this.safeClone(CORPO_INTEGRADO), { alerts, userQuestions })
            );
            ...
            this.status = 'error';
            console.error('❌ CRITICAL ERROR IN MASTER ORCHESTRATOR:', error);
            logger.critical(
                ORQUESTRADOR_NOME,
    ...
    /**
     * Clona objeto com fallback seguro em caso de erro (ex: DataCloneError)
     */
    safeClone(obj) {
        try {
            ...
            logger.warn(
                ORQUESTRADOR_NOME,
                'Falha ao clonar objeto para isolamento (usando referência original)',
                { error: error.message }
            );
            return obj;
        }
