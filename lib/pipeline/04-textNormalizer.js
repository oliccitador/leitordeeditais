...
class TextNormalizer {
    ...
    async normalize(ocrResult) {
        try {
            ...
            // 🔍 DEBUG: Log entrada TextNormalizer (console + arquivo)
    ...
    detectPageNumbers(pages) {
        ...
        // 🔧 FIX: Regex mais específicas para evitar casar com números de edital/processo (XXX/2025)
        const patterns = [
            /p[aá]gina\s*\d+/i,
            /p[aá]g\.\s*\d+/i,
            // Apenas números pequenos (1-3 dígitos) / números pequenos (1-3 dígitos) - para páginas
            // Exclui formato de ano como 042/2025 ou 067/2025
            /\b\d{1,2}\s*\/\s*\d{1,3}\b(?!\s*20)/,
            /\d+\s+de\s+\d+/i
        ];
    ...
    normalizeText(text) {
        ...
        // 1. Remove caracteres null e de controle (PREVARVANDO \n, \r e \t)
        // BUG FIX: regex anterior removia \n (ASCII 10) e \r (ASCII 13)
