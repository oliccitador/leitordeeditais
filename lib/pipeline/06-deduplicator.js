...
class Deduplicator {
    ...
    async deduplicate(indexedDocs) {
        try {
            ...
            // 🔍 DEBUG: Log SAÍDA Deduplicator (rastrear keys) - compatível ESM
            import('fs').then(fsModule => {
                import('path').then(pathModule => {
                    const fsLib = fsModule.default || fsModule;
                    const pathLib = pathModule.default || pathModule;
                    const logLine = `[06-OUT] ${JSON.stringify({
    ...
    findDuplicateGroups(docs) {
        ...
        for (let i = 0; i < docs.length; i++) {
            for (let j = i + 1; j < docs.length; j++) {
                ...
                // 🛡️ GUARDRAIL: Documentos de tipos diferentes NUNCA são duplicatas (ex: Edital vs TR)
                // Se o classificador os separou, eles devem coexistir para análise de divergência.
