...
class DocumentFusion {
    ...
    async fuse(uniqueDocs, loteId, classification) {
        try {
            ...
            // 🔍 DEBUG: Log entrada DocumentFusion (console + arquivo)
            ...
            const dbg = (tag, obj) => {
                ...
                console.log(logLine);
                try {
                    fsSync.appendFileSync(pathLib.join(process.cwd(), 'debug-ocr-pipeline.log'), logLine + '\n');
                } catch (e) { /* ignore */ }
            };
    ...
    buildMetadata(docs, segments, globalLines) {
        ...
        // 🔍 DEBUG: Log buildMetadata (console + arquivo)
        // Importar fs/path dinamicamente (compatível ESM)
        import('fs').then(fsModule => {
            import('path').then(pathModule => {
                const fsLib = fsModule.default || fsModule;
                const pathLib = pathModule.default || pathModule;
                const logLine = `[07-META] ${JSON.stringify({
                    docQs: docQs.length,
                    segQs: segQs.length,
                    docSample: docQs.slice(0, 3),
                    segSample: segQs.slice(0, 3),
                    usingSource: docQs.length > 0 ? 'docs' : 'segments'
                })}`;
                console.log(logLine);
                try {
                    fsLib.appendFileSync(pathLib.join(process.cwd(), 'debug-ocr-pipeline.log'), logLine + '\n');
                } catch (e) { /* ignore */ }
            });
        });
