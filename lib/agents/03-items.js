...
class ItemClassifier {
    ...
    async extractItems(corpoIntegrado) {
        ...
        // Regex patterns para detectar itens - Ajustados
        const itemPatterns = [
            // Padrão flutuante: \b para pegar numeral isolado mesmo no meio da linha OCR
            { pattern: /\b(\d{1,2})\s+([A-ZÁÉÍÓÚÂÊÔÃÕÇ]{3,}[^\n]{10,})/gm, type: 'SIMPLE_START' },
            // Padrão explícito ITEM
            { pattern: /(?:^|\n)\s*ITEM\s+(\d{1,3})\s*[-:.]?\s*([^\n]{10,300})/gi, type: 'ITEM' },
            // Padrão hierárquico
        ...
        const exclusions = /PREÂMBULO|CLÁUSULA|SEÇÃO|CAPÍTULO|OBJETO|DISPOSIÇÕES|ANEXO|CONTRATO|ATA|LEI|DECRETO|PORTARIA|EDITAL|DATA|LOCAL|PRAZO|JANEIRO|FEVEREIRO|MARÇO|ABRIL|MAIO|JUNHO|JULHO|AGOSTO|SETEMBRO|OUTUBRO|NOVEMBRO|DEZEMBRO|PÁGINA|FOLHA/i;
            ...
            for (const { pattern, type } of itemPatterns) {
                ...
                for (const match of matches) {
                    ...
                    if (type === 'SIMPLE_START') {
                        ...
                        // logger.info(AGENT_ID, `[DEBUG] SIMPLE_START MATCH: ${numero} - ${descricao.substring(0,30)}`);
                    }

                    const key = `${numero}-${descricao.substring(0, 30)}`;
                    if (processed.has(key) || descricao.length < 5) continue;

                    processed.add(key);

                    // Contexto completo (próximas 5 linhas para garantir captura de Qtd/Unid)
                    ...
                    if (quantidade === "SEM DADOS NO ARQUIVO") {
                        logger.warn(AGENT_ID, `[DEBUG] Falha Qtd Item ${numero}. Context: "${fullContext.substring(0, 50)}..."`);
                    }
