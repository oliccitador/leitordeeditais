...
class DivergenceScanner {
    ...
    async extractFieldValues(corpoIntegrado, documentsByType) {
        ...
        const patterns = {
            prazo_entrega: /\bprazo\s+de\s+entrega\b[^0-9]*(\d+)\s+(dias?|horas?)/i,
            prazo_pagamento: /\bprazo\s+de\s+pagamento\b[^0-9]*(\d+)\s+dias?/i,
            item_quantidade: /\bquantidade\b[^0-9]*(\d+)/i,
