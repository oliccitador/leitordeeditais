...
class ComplianceChecker {
    ...
    async extractRequirements(corpoIntegrado) {
        ...
        // ✅ DEDUPLICAÇÃO: Remover requisitos duplicados
        const requisitosUnicos = [];
        const descricaoSet = new Set();

        for (const req of requisitos) {
            // Chave única: categoria + descrição
            const chave = `${req.categoria}:${req.descricao}`;

            if (!descricaoSet.has(chave)) {
                descricaoSet.add(chave);
                requisitosUnicos.push(req);
            } else {
                // Se já existe, adiciona a evidência ao requisito existente
                const reqExistente = requisitosUnicos.find(r =>
                    r.categoria === req.categoria && r.descricao === req.descricao
                );
                if (reqExistente && req.origens.length > 0) {
                    reqExistente.origens.push(...req.origens);
                }
            }
        }

        logger.info(AGENT_ID, `Deduplicação: ${requisitos.length} → ${requisitosUnicos.length} requisitos únicos`);
        ...
        return { requisitos: requisitosUnicos, evidence, conflicts };
    }
