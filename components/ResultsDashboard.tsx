...
function HabilitacaoSection({ result }: { result: any }) {
    ...
    const requisitos = agent4?.dados?.requisitos || [];

    // Checklist é um objeto com categorias: { fiscal: [...], trabalhista: [...], etc }
    const categorias = Object.entries(checklist);
    ...
    return (
        <div className="space-y-4">
            ...
            {categorias.length === 0 ? (
                ...
                <div className="space-y-6">
                    {categorias.map(([categoria, items]: [string, any]) => (
                            ...
                            <ul className="space-y-2">
                                {Array.isArray(items) ? items.map((item: string, i: number) => (
                                    <li key={i} className="flex items-start gap-2">
                                        ...
                                        <span className={item === 'SEM DADOS NO ARQUIVO' ? 'text-gray-400 italic' : ''}>
                                            {item}
                                        </span>
                                    </li>
                                )) : <li className="text-gray-400 italic">SEM DADOS NO ARQUIVO</li>}
                            </ul>
                        </div>
                    ))}
                </div>
            ...
            {/* Requisitos com detalhes (se houver) */}
            {requisitos.length > 0 && (
                <div className="mt-6">
                    <h3 className="font-semibold text-lg mb-3">Detalhes dos Requisitos:</h3>
                        ...
                        {requisitos.map((req: any, i: number) => (
                            <li key={i} className="flex items-start gap-3 p-3 bg-gray-50 rounded">
                                ...
                                <div className="flex-1">
                                    <p className="font-medium">{req.descricao}</p>
...
function TecnicoSection({ result }: { result: any }) {
    ...
    const resumo = agent5?.dados?.resumo || {};
        ...
        <div className="space-y-4">
            ...
            {Object.keys(resumo).length > 0 && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-blue-50 rounded-lg">
                    <div>
                        <p className="text-sm text-gray-600">Exige Atestado</p>
                        <p className="font-bold">{resumo.exige_atestado ? '✅ SIM' : '❌ NÃO'}</p>
                    </div>
                    <div>
                        <p className="text-sm text-gray-600">Exige Normas</p>
                        <p className="font-bold">{resumo.exige_normas ? '✅ SIM' : '❌ NÃO'}</p>
                    </div>
                    <div>
                        <p className="text-sm text-gray-600">Exige Visita</p>
                        <p className="font-bold">{resumo.exige_visita ? '✅ SIM' : '❌ NÃO'}</p>
                    </div>
                    <div>
                        <p className="text-sm text-gray-600">Gatilhos Impugnação</p>
                        <p className={`font-bold ${resumo.gatilhos_impugnacao > 0 ? 'text-red-600' : 'text-green-600'}`}>
                            {resumo.gatilhos_impugnacao || 0}
                        </p>
                    </div>
                </div>
            )}
            ...
            {requisitos.length === 0 ? (
                ...
                <ul className="space-y-3">
                    {requisitos.map((req: any, i: number) => (
                        <li key={i} className="p-4 bg-gray-50 rounded border-l-4" style={{
                            borderLeftColor: req.nivel_risco === 'ALTO' ? '#ef4444' :
                                req.nivel_risco === 'MEDIO' ? '#f59e0b' : '#10b981'
                        }}>
                            <div className="flex items-start justify-between">
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className={`px-2 py-1 rounded text-xs font-bold ${req.nivel_risco === 'ALTO' ? 'bg-red-100 text-red-800' :
                                                req.nivel_risco === 'MEDIO' ? 'bg-yellow-100 text-yellow-800' :
                                                    'bg-green-100 text-green-800'
                                            }`}>
                                            RISCO {req.nivel_risco}
                                        </span>
                                    </div>
                                    <p className="font-medium">{req.criterio}</p>
                                    {req.trecho_literal && (
                                        <p className="text-sm text-gray-600 mt-2 italic">
                                            "{req.trecho_literal.substring(0, 150)}..."
                                        </p>
                                    )}
                                    {req.justificativa_gatilho && req.justificativa_gatilho !== 'SEM DADOS NO ARQUIVO' && (
                                        <p className="text-sm text-gray-700 mt-2">
                                            <strong>Justificativa:</strong> {req.justificativa_gatilho}
                                        </p>
                                    )}
                                </div>
                                {req.gatilho_impugnacao && (
                                    <span className="px-3 py-1 rounded text-sm font-bold bg-red-500 text-white ml-4">
                                        ⚠️ GATILHO IMPUGNAÇÃO
                                    </span>
                                )}
                            </div>
                        </li>
